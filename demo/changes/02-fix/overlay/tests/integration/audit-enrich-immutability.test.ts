...
describe("Audit Enrichment Immutability (invariant floor)", () => {
  ...
  // Counter-example added during closure of the 01-audit-mutation incident.
  // It catches the *class* — any in-place write — not the exact assertion that
  // happened to fail first. A frozen original turns mutation into a thrown
  // TypeError under ES-module strict mode, and repeated enrichment proves each
  // call only ever appends.
  it("counter-example: a frozen original cannot be rewritten; repeated enrichment only appends", async () => {
    const sessionId = `sess_${uuidv4().slice(0, 8)}`;
    const original = pendingEvent(sessionId);
    await store.storeEvent(original);

    const stored = (await store.getSession(sessionId))[0];
    Object.freeze(stored.action);
    Object.freeze(stored.policy_detail);
    Object.freeze(stored);

    const enrich = (effect: string) =>
      handleEnrich(
        JSON.stringify({
          session_id: sessionId,
          tool: "Bash",
    ...
    await expect(enrich("still running")).resolves.toMatchObject({ status: 200 });
    await expect(enrich("executed (exit 0)")).resolves.toMatchObject({ status: 200 });

    const after = await store.getSession(sessionId);
    expect(after.length).toBe(3); // original + two appended enrichments
    expect(after[0].action.observed_effect).toBe("pending");
    expect(after.filter((e) => e.correlation_id === original.event_id).length).toBe(2);
  });
});
